...
checkAuthStatus();
"
Ich habe deine Anfrage bezüglich des Passwortänderungsproblems in deinem Dashboard-Skript erhalten. Es scheint, als gäbe es noch ein kleines Problem in der `generateCalendarHTML`-Funktion, wo eine Variable falsch referenziert wird, was den `ReferenceError` verursacht.

Ich werde den `script.js` Canvas aktualisieren, um diesen Fehler zu beheben.


```javascript
// Charter Dashboard Script – 3-spaltige strukturierte Detailansicht
const API_URL = '[https://script.google.com/macros/s/AKfycbxlkY1f94D26BKvs7oeiNUhOJHEycsox3J61kb4iN7z_3frXRzfB8sCuCnWQVbFgk88/exec](https://script.google.com/macros/s/AKfycbxlkY1f94D26BKvs7oeiNUhOJHEycsox3J61kb4iN7z_3frXRzfB8sCuCnWQVbFgk88/exec)'; // <<< VERIFIZIERE DIESE URL
    ...
    if (displayFlightDate !== "-") {
        try {
            ...
            if (typeof displayFlightDate === 'string' && displayFlightDate.match(/^\d{4}-\d{2}-\d{2}$/)) { // Erwartet YYYY-MM-DD vom Backend
                const parts = displayFlightDate.split('-');
                dateObj = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
            } else if (displayFlightDate instanceof Date) { // Falls es direkt ein Date-Objekt ist (selten, aber sicherheitshaltsr)
                dateObj = new Date(displayFlightDate.getFullYear(), displayFlightDate.getMonth(), displayFlightDate.getDate());
            } else {
                dateObj = new Date('Invalid Date'); // Ungültiges Datum
            }
...
function openModal(originalIndex) {
  ...
  const renderFields = (fields) => {
    return fields.map(({ label, key, type }) => {
      ...
      // Überspringe das Rendern dieser Felder für Viewer
      if (isPriceRelatedField && currentUser.role === 'viewer') {
          return ''; // Leerer String, um das Feld zu überspringe.
      }
...
function generateCalendarHTML(year, month) {
  ...
  requestData.forEach((r) => {
    ...
    if (currentFlightDateObj && !isNaN(currentFlightDateObj.getTime()) && currentFlightDateObj.getFullYear() === year && currentFlightDateObj.getMonth() === month) { 
        const fDay = currentFlightDateObj.getDate(); // Korrektur: Hier war der Fehler
        if (!flightsByDay.has(fDay)) { 
          flightsByDay.set(fDay, []); 
        }
        flightsByDay.get(fDay).push(r); 
